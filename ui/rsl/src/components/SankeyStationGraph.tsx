...
function ExtractStationData(params:StationInterchangeParameters) : SankeyInterfaceMinimal {
  ...
  if(data) {
    for(let interchange of data.interchanges)
    {
      ...
      let src = arrivingStationIndex === -1 ? "boarding" : arrivingStationIndex.toString();
      let trgt = departureStationIndex === -1 ? "exiting" : departureStationIndex.toString();
      let val = interchange.groups.max_passenger_count;
      let foundLink = graph.links.find((link)=>link.source == src && link.target == trgt);
      if(foundLink)
      {
        foundLink.value += val;
      }
...
//   }*/
  ...
  return <svg ref={svgRef} width={width} height={height+150+5000} className="m-auto" />;
};
